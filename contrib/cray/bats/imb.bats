...
# RC                                                                                                                                     
@test "imb_MPI1 allscatter 40 ranks, 10 ranks per node using RC verbs" {
        run $CONTRIB_BIN/logwrap -w ${BATS_TEST_LOGFILE} -- \
                $(batch_launcher 40 10) timeout 300 "$IMB_BUILD_PATH/imb_MPI1 allscatter"
        [ "$status" -eq 0 ]
}

# XRC
@test "imb_MPI1 allscatter 40 ranks, 10 ranks per node using XRC verbs" {
        FI_OFI_RXM_USE_SRX=1 FI_VERBS_PREFER_XRC=1 run $CONTRIB_BIN/logwrap -w ${BATS_TEST_LOGFILE} -- \
                $(batch_launcher 40 10) timeout 300 "$IMB_BUILD_PATH/imb_MPI1 allscatter"
        [ "$status" -eq 0 ]
}
# RC                                                                                                                                     
@test "imb_MPI1 allscatterv 40 ranks, 10 ranks per node using RC verbs" {
        run $CONTRIB_BIN/logwrap -w ${BATS_TEST_LOGFILE} -- \
                $(batch_launcher 40 10) timeout 300 "$IMB_BUILD_PATH/imb_MPI1 allscatterv"
        [ "$status" -eq 0 ]
}

# XRC
@test "imb_MPI1 allscatterv 40 ranks, 10 ranks per node using XRC verbs" {
        FI_OFI_RXM_USE_SRX=1 FI_VERBS_PREFER_XRC=1 run $CONTRIB_BIN/logwrap -w ${BATS_TEST_LOGFILE} -- \
                $(batch_launcher 40 10) timeout 300 "$IMB_BUILD_PATH/imb_MPI1 allscatterv"
        [ "$status" -eq 0 ]
}
